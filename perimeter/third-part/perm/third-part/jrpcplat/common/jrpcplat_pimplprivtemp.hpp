...
*/
/* ///////////////////////////////////////////////////////////////////// */
#ifndef  JRPCPLAT_PIMPLPRIVTEMP_HXX
#define  JRPCPLAT_PIMPLPRIVTEMP_HXX
...
#define JRPCPLAT_PImplPrivTemp_PtrCast( t, o ) (  reinterpret_cast< t >( reinterpret_cast< intptr_t >( o ))  )
#define JRPCPLAT_PImplPrivTemp_IntPtrCast( o ) (  reinterpret_cast< intptr_t >( o ))


#ifndef JRPCPLAT_PImplPrivTemp_new
#define JRPCPLAT_PImplPrivTemp_new( t, ... )  ( new t( __VA_ARGS__ ))
#endif

#ifndef JRPCPLAT_PImplPrivTemp_delete
#define JRPCPLAT_PImplPrivTemp_delete( o, t )  do{ delete JRPCPLAT_PImplPrivTemp_PtrCast( t*,  o ); }while(0)
#endif
...
public:   
    ...
    static T*    createInstance ( )           { return JRPCPLAT_PImplPrivTemp_new( T ); }
    static T*    createInstance ( void *ref ) { return JRPCPLAT_PImplPrivTemp_new( T,  * JRPCPLAT_PImplPrivTemp_PtrCast( T*, ref ) ); }
    static T*    addRef    ( void *cd )       {
        if ( cd != nullptr ) {
            JRPCPLAT_PImplPrivTemp_PtrCast( T*, cd )->addRefCntr( );
        }
        return  JRPCPLAT_PImplPrivTemp_PtrCast( T*, cd );
    }
...
#define JRPCPLAT_PImplPrivTemp_AttachPtr( t, s, d )\
    t :: attach( JRPCPLAT_PImplPrivTemp_PtrCast( void**, s ), JRPCPLAT_PImplPrivTemp_PtrCast( void**, d ))

#define JRPCPLAT_PImplPrivTemp_BuildIfNull( t, d ) \
    t :: buildIfNull( JRPCPLAT_PImplPrivTemp_PtrCast( void**, d ) )
...
T*   PImplPrivTemp<T> :: buildIfNull( void **w_obj )
{
    ...
    //return ( T *)( *w_obj );
    return JRPCPLAT_PImplPrivTemp_PtrCast( T*, *w_obj );
}
...
bool  PImplPrivTemp<T> :: releaseRef ( void *cd )
{
    ...
    if ( cd != nullptr ) {
        ...
        if ( JRPCPLAT_PImplPrivTemp_PtrCast( T*, cd )->releaseRefCntr() == 0 ) {
            JRPCPLAT_PImplPrivTemp_delete( cd, T );
            is_released = true;
        }
    }   	
    return is_released;
...
T *    PImplPrivTemp<T> :: duplicateIfShared( void **w_obj )
{
    ...
    if ( this->currRefCntr( ) == 1 ) { return JRPCPLAT_PImplPrivTemp_PtrCast( T *, *w_obj ); }
...
//    if ( (( T *) *w_obj)->currRefCntr( ) == 1 ) { return ( T *)( *w_obj ); }
    if ( JRPCPLAT_PImplPrivTemp_PtrCast( T*, *w_obj)->currRefCntr( ) == 1 ) {
        return JRPCPLAT_PImplPrivTemp_PtrCast( T*, *w_obj );
    }
