#ifndef PERM_MEMCNTR_CXX
#define PERM_MEMCNTR_CXX

#include "perm_memcntr.hxx"
#include "perm_logging.hxx"
#include "perm_semtemp.hpp"
#include "perm_dequetemp.hpp"
#include "perm_dyncinit_priv.hxx"
...
class PERM_HIDDEN MemCntrConstStrCmp {
public:
...
class PERM_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class PERM_HIDDEN MemCntrRecAction {
...
typedef FcPerm::Deque<MemCntrRec>        MemCntrRecQueue;
typedef FcPerm::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class PERM_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  perm_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = perm_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *perm_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
#else
    PERM_UNUSED(file); PERM_UNUSED(func);
    PERM_UNUSED(ln);   PERM_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            perm_fatal("allocated memory, but return nullptr!");
        } else {
            perm_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = perm_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = perm_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
