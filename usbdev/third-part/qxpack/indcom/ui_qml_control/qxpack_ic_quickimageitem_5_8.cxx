#ifndef  QXPACK_IC_QUICKIMAGEITEM_5_8_CXX
#define  QXPACK_IC_QUICKIMAGEITEM_5_8_CXX
...
void       IcQuickImageItem :: qsgRemapTexture( QSGNode *node, const QSizeF &im_sz )
{
    ...
    QSGImageNode *tex_node = static_cast<QSGImageNode*>(node);
...
{
    QSGImageNode *node = Q_NULLPTR;
    if ( old == Q_NULLPTR ) { return old; }
    node = static_cast< QSGImageNode *>( old );
...
void      IcQuickImageItem :: qsgEnsureFlipMode ( QSGNode *old )
{
    ...
    QSGImageNode::TextureCoordinatesTransformMode  trans_mode = QSGImageNode::NoTransform;
    if ( t_this->cmpReqAndCurrFlipV() ) {
        if ( t_this->currFlipV()) { trans_mode |= QSGImageNode::MirrorVertically; }
        is_need_set = true;
    }
    if ( t_this->cmpReqAndCurrFlipH() ) {
        if ( t_this->currFlipH()) { trans_mode |= QSGImageNode::MirrorHorizontally; }
        is_need_set = true;
    }
...
QSGNode*  IcQuickImageItem :: updatePaintNode ( QSGNode *old,UpdatePaintNodeData*)
{
    ...
    if ( t_this->m_im_source_req_cntr.loadAcquire() > 0 ) {
        ...
        qsgRemapTexture( old, t_this->lastImageSize() ); // since 0.5.12 added
        qsgEnsureFlipMode( node );                       // since 0.5.12 added
        return old;
    }
